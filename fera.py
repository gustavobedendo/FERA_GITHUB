...
class MainWindow():    
    ...
    def __init__(self, testar1=False, testar2=False):
        global loading_dialog, loading_progress, buscasiniciais, buscasiniciaisok
        loading_dialog = None
        loading_progress = None
        buscasiniciais = global_settings.contador_buscas_incr -1
        buscasiniciaisok = 0
        ...
        try:
            ...
            traceback.print_exc()
        if(testar1 or testar2):
            if(len(naoindexados)>0):
                global_settings.allok = 75
                global_settings.on_quit()
                return
        self.initUI(testar1, testar2)
        if(fera_config.get_dontshow()==0):
            global_settings.popup_whatsnew = whats_new.create_popup()
        ...
        if(not testar1 and not testar2 and plt == 'Linux' and not _executable_exists("xclip")):
        ...
        if(fera_config.get_dontshow()==0):
            global_settings.popup_whatsnew.lift()
            global_settings.popup_whatsnew.focus_set()
        
        if(testar1):
            
            self.searchTerm(termo="tabela 1:", tipobusca="LIKE", testar=testar1)
            self.searchTerm(termo="teste:", tipobusca="MATCH", testar=testar1)
            self.searchTerm(termo="teste", tipobusca="REGEX", testar=testar1)
            
        if(testar1 or testar2):
            testar = testar1 or testar2
            global_settings.root.after(10000, lambda testar=testar: self.wait_results(testar))
            self.inittest = time.time()
            
            
            
        
    def wait_results(self, testar=False):
        try:
            if(time.time() - self.inittest>6000):
                global_settings.allok = 73
            if(global_settings.allok>0):
                global_settings.on_quit()
                return
            if(self.treeviewSearches.winfo_exists()):
                resultados = len(self.treeviewSearches.tag_has("termosearching")) + len(self.treeviewSearches.tag_has("termoinprocess"))
                if(len(self.treeviewSearches.tag_has("termowitherror"))>0):
                    global_settings.allok = 74
            print('termosearching', resultados)
            if(resultados>0):
                pass #global_settings.root.after(10000, self.wait_results)
            else:
                if(self.treeviewSearches.winfo_exists()):
                    resultados = len(self.treeviewSearches.tag_has("termosearch"))
                    print('termosearch', resultados)
                    try:
                        termos = self.treeviewSearches.get_children('')
                        for termo in termos:
                            if('LIKE' not in self.treeviewSearches.item(termo, 'text')):
                                continue
                            equipamentos = self.treeviewSearches.get_children(termo)
                            for equipamento in equipamentos:
                                pdfs = self.treeviewSearches.get_children(equipamento)
                                print(equipamento, len(pdfs))
                                if(len(pdfs)==0):
                                    global_settings.allok = 75
            ...
            time.sleep(1)
            if(global_settings.exitFlag):
                pass
            else:
                traceback.print_exc()
                global_settings.allok = 76
        finally:
            global_settings.root.after(10000, self.wait_results)
            
            
 

    def initUI(self, testar1=False, testar2=False):        
        ...
        self.checkUpdates()
        self.treeSeachAfter(testar1, testar2)
        self.checkPages()
    ...
    def free_searches(self, testar=False):
        try:  
            if(global_settings.indexing_thread!=None and global_settings.indexing_thread.is_alive()):
                proc = None
                ...
                if(len(naoindexados)> 0):
                    ...
                    janela = utilities_general.popup_window(texto, False)
                    utilities_general.below_right(janela,self.docInnerCanvas.winfo_rooty())
                    if(not testar):
                        global_settings.root.wait_window(janela)
                    self.notebook.tab(1, state="disabled")
                    self.simplesearch.config(state='disabled')
                    self.searchbutton.config(state='disabled')
                    self.nhp.config(state='disabled')
    ...
    def treeSeachAfter(self, testar1=False, testar2=False):
        global loading_dialog, loading_progress, buscasiniciaisok
        resultsearch = None
        intervalo = 300
        try:
            continueres = None
            ...
            while True: 
                if(buscasiniciaisok==buscasiniciais):
                    try:
                        global_settings.splash_window.window.destroy()
                    except:
                        None
                if (((endtime-starttime)/1000000) > 60):
                    #print(contagem)
                    break
                endtime = time.process_time_ns()
                if(not global_settings.result_queue.empty()):
                    try:
                        ...
                        treeview_to_manipulate = self.treeviewSearches
                        result_to_check = res[1]
                        if(isinstance(res[1], list)):
                            result_to_check = res[1][0]
                        if(result_to_check.fonte=='arquivo'):
                            treeview_to_manipulate = self.treeviewSearchesFiles
                            #continue
                        ...
                        if(res[0]==0):
                            ...
                            if(not treeview_to_manipulate.exists(iidx)):
                                treeview_to_manipulate.insert(parent='', index=-1*int(idtermo), iid=iidx, text=termo.upper(), tag=('termosearching',), image=global_settings.lupa, \
                            ...
                            if(treeview_to_manipulate.exists(idx)):
                                treeview_to_manipulate.item(idx, tags=("termoinprocess",))
                        #terminou de receber resultados de buscas para o termo
                            ...
                            tipobusca = resultsearch.tipobusca
                            treeview_to_manipulate.item(idx, tags=("termosearch",))
                            termo  = resultsearch.termo
                            th = utilities_general.countChildren(treeview_to_manipulate, idx)   
                            treeview_to_manipulate.item(idx, text=termo.upper() + ' (' + str(th) + ')'  + " - "+tipobusca)  
                            treeview_to_manipulate.item(idx, tags=("termosearch",))
                            valores = treeview_to_manipulate.item(idx, 'values')
                            treeview_to_manipulate.item(idx, values=(valores[0], valores[1], th, termo.upper(), tipobusca,))
                            
                            
                        elif(res[0]==1):
                            starttime2 = time.process_time_ns()
                            try:
                                for resultsearch in res[1]:
                                        
                                #resultsearch = res[1]
                                    idtermo = resultsearch.idtermo
                                    if(not treeview_to_manipulate.exists('t'+idtermo)):
                                        continue
                                    else:
                                        termo = resultsearch.termo
                                        ...
                                        te = 'te'+resultsearch.idtermo+resultsearch.parent_alias
                                        ...
                                        if(not treeview_to_manipulate.exists(te)):  
                                            index_insert = self.insertIndex(treeview_to_manipulate, t, resultsearch.parent_alias, 0)
                                            treeview_to_manipulate.insert(parent=t, index=index_insert, iid=te, \
                                                                         text=resultsearch.parent_alias, tag=('eqsearch'), image=global_settings.imagereportb, \
                                                                             values=(pathpdfbase, resultsearch.idpdf,))                                     
                                        
                                        if(False and global_settings.info_index_boolean):
                                            te_relatorio = 'te_relatorio'+resultsearch.idtermo+resultsearch.parent_alias
                                            te_content = 'te_content'+resultsearch.idtermo+resultsearch.parent_alias
                                            if(not treeview_to_manipulate.exists(te_relatorio)): 
                                                treeview_to_manipulate.insert(parent=te, index='end', iid=te_relatorio, \
                                                                            text="Relatório(s)", tag=('eqsearch'), image=global_settings.imagereportb, \
                                                                                values=(pathpdfbase, resultsearch.idpdf,)) 
                                            erro = ""
                                            naodisp = ""
                                            if(global_settings.info_index[resultsearch.parent_alias][0]=="Não"):
                                                erro = "error"
                                                naodisp = " (Não indexado)"
                                            if(not treeview_to_manipulate.exists(te_content)): 
                                                treeview_to_manipulate.insert(parent=te, index='end', iid=te_content, \
                                                                            text=f"Arquivos{naodisp}", tag=(f'arqsearch{erro}'), image=global_settings.imagereportb, \
                                                                                values=(pathpdfbase, resultsearch.idpdf,)) 
                                            te = te_relatorio
                                        ...
                                        if(tptoc!=None):
                                            ...
                                            if(not treeview_to_manipulate.exists(tptoc)):
                                                 treeview_to_manipulate.insert(parent=te, iid=tptoc, text=tocname, index='end', tag=('relsearchtoc'), values=(tocname,))    
                                            
                                            idx = treeview_to_manipulate.insert(parent=tptoc, index='end', text=snippettotal, tag='resultsearch', \
                                            ...
                                            if(tamanho>self.maiorresult):
                                                ...
                                                treeview_to_manipulate.column("#0", width=self.maiorresult, stretch=True, minwidth=self.maiorresult, anchor="w")
                                            #resultsearch.snippet = ""
                                            global_settings.searchResultsDict[idx] = resultsearch
                                        else:
                                            idx = treeview_to_manipulate.insert(parent=te, index='end', text=' '+str(pagina+1)+' - '+snippet, tag='resultsearch', \
                                                                                image=global_settings.snippet, values=(resultsearch.snippet[0], resultsearch.snippet[1], \
                                                                                                            resultsearch.snippet[2],str(pagina+1),))
                                            snippettotal = str(pagina+1)+' - '+snippet                                        
                                            tamanho = len(snippettotal)*4+150                                        
                                ...
                                if(testar1 or testar2):
                                     global_settings.allok = 77
                                utilities_general.printlogexception(ex=ex)
                    except Exception as ex:
                        if(testar1 or testar2):
                            global_settings.allok = 78
                        utilities_general.printlogexception(ex=ex)
                else: 
                    #if(contagem>0):
                    #    print(contagem)
    ...
    def treeview_eqs_right(self, event=None):
        ...
        try:
            ...
            if(False and self.treeviewEqs.tag_has('equipmentlp', iid)):
                self.eqmenu = tkinter.Menu(global_settings.root, tearoff=0)
    ...
    def exportHashListToFile(self):
        try:
            iid = self.treeviewObs.selection()
    
            lista = []
            self.descendants_obsitem(iid, lista)
            obsbypdf = {}
            for item, pdf in lista:
                if(pdf not in obsbypdf):
                    obsbypdf[pdf] = []
                observation = self.allobsbyitem[item]
                obsbypdf[pdf].append(observation)
            total_hashes = []
            for pdf in obsbypdf:
                total_hashes.extend(utilities_general.get_text_from_obs(obsbypdf[pdf]))
            if(len(total_hashes)==0):
                messagebox.showerror("Error", "Não foram identificados hashes nas Observações selecionadas.")
                return
            else:
                        ...
                        for link in links:
                            ...
                            try:
                                ...
                                for obs in pdfs[temppdf][page]:
                                    pm = (r.y0 + r.y1) / 2
                                    if((r.y0 >= obs[0] and r.y1 <= obs[1]) or\
                                        (r.y0 < obs[0] and r.y1 > obs[1]) or\
                                        (obs[0] <= r.y0 and obs[1] <= r.y1 and obs[1] >= r.y0) or\
                                        (obs[0] >= r.y0 and obs[1] >= r.y1 and obs[0] <= r.y1) or\
                                        (pm >= obs[0] and pm <= obs[1])):
    ...
    def treeview_obs_right(self, event=None):        
        ...
        if(self.treeviewObs.tag_has('obscat',iid)):
            ...
            self.copyobsto.add_command(label="Clipboard (RTF)", image=global_settings.copycat, \
                                       compound='left', command=lambda: self.copy_obs_rtf(False, False))
            self.copyobsto.add_command(label="Clipboard (RTF) - Intervalo completo", image=global_settings.copycat, \
                                       compound='left', command=lambda: self.copy_obs_rtf(False, True))
            ...
            self.copyobsto.add_command(label="Clipboard (Imagens em Tabela - RTF)", image=global_settings.copycat, \
                                       compound='left', command=lambda: self.export_images_table_rtf(False, False))
            self.copyobsto.add_command(label="Clipboard (Imagens em Tabela - RTF) - Intervalo completo", image=global_settings.copycat, \
                                       compound='left', command=lambda: self.export_images_table_rtf(False, True))
            ...
            self.editdelcat.add_command(label="Exportar lista de hashes", image=global_settings.imtoHash, \
                                        compound='left', command= self.exportHashListToFile)
...
"{{\\f2\\fswiss\\fprq2\\fcharset0 Arial;}}}}{{\\colortbl;\\red240\\green240\\blue240;\\red221\\green221\\blue221;\\red255\\green255\\blue255;\\red190\\green0\\blue0;}} {}}}")\
            ...
            try:
                ...
                None
    def copy_obs_rtf_single_table(self, withnotes=False, fullinterval=False):  
        pathdocespecial = None
        docespecial = None
        pagatual = None
        textonatabela = ""
            ...
            for iid in self.treeviewObs.selection():
                ...
                if(tiposelecao=='texto'):
                    if(textoselecao.endswith("(...)\\line")):
                        textoselecao += f"\\f2\\fs20(Fls. {observation.paginainit+1})\\line"
                    else:
                        textoselecao += f"\\line\\f2\\fs20(...) (Fls. {observation.paginainit+1})\\line"
                else:
                    if(textoselecao.endswith("(...)\\line")):
                        textoselecao += f"\\f2\\fs20(Fls. {observation.paginainit+1})\\line"
                    else:
    ...
    def export_images_table_rtf(self, withnotes=False, fullinterval=False):    
        utilities_export.Export_Images_To_Table(self, withnotes)
        
    def copy_obs_rtf(self, withnotes=False, fullinterval=False):  
        selected_obs = self.treeviewObs.selection()
        if(len(selected_obs)>1):
            self.copy_obs_rtf_single_table(withnotes, fullinterval)
            ...
            if(extension.lower() in global_settings.listavidformats):
                ...
                if(os.path.exists("teste.png")): 
                    ...
                    with Image.open("teste.png") as imx:
                        imx.thumbnail(size, Image.LANCZOS)
         ...
         try:
             ...
             if(tiposelecao=='area'):                 
                 ...
                 for pagina in range(pinit, pfim+1):
                     ...
                     box = fitz.Rect(min(p0x, p1x), min(p0y, p1y), max(p1x, p0x), max(p1y, p0y))
                 ...
                 if(len(images) > 0):
                     ...
                     if platform.system() == 'Windows' or  platform.system() == 'Linux':    # Windows
                        ...
                        if(pmin!=None and pmax!=None and pmin!=pmax):
                            docpagina = "{{\\fs22\\f2\\b{{ Relat}}\\\'D3rio \\\'22{}\\\'22 -- Fls. {} a {}}}".format(docname, pmin+1, pmax+1)
                        else:
                            docpagina = "{{\\fs22\\f2\\b{{ Relat}}\\\'D3rio \\\'22{}\\\'22 -- Fls. {}}}".format(docname, pagina+1)   
                        docpagina = docpagina.upper()
                        #docpagina = "{{\\fs22\\f2{{ Relat}}\\\'F3rio \\\'22{}\\\'22 -- Fls. {}}}".format(docname, pagi)
                        textonatabela += ("\\par\\trowd\\clbrdrb\\brdrs\\clbrdrt\\brdrs\\clbrdrl\\brdrs\\clbrdrr\\brdrs\\trautofit1\\intbl\\clftsWidth3\\clwWidth9070\\cellx9070{{\\cbpat2\\qc\\loch\\b\\fs22\\f2 TABELA }}{{\\hich\\af2\\af2\\loch\\b\\f2\\f2\\fs22\\loch\\field{{\\*\\fldinst{{SEQ Tabela\\\\* ARABIC }}}}{{\\fldrslt{{ }}}}}}"+\
                        "{{\\f2\\b\\fs22\\qc:{}}}\\f2\\fs20\\cell\\row"+\
                                       ...
                                       for char in span['chars']:                                                 
                                           ...
                                           if((float(quad[0])+float(quad[2]))/2 <= p1x):  
                                               ...
                                               if(ord(c)>=55296 and primeiroemoji):
                                                   ...
                                                   c = ' ' + c
                                               linhaorg += c
                                if(linhaorg.startswith("Anexo:") or linhaorg.startswith("Transcrição Automatizada:")):
                                    linhaorg = "↳ "+linhaorg
                                if("Respondendo mensagem de <" in linhaorg):
                                    print("aqui1")
                                    respondendo = True
                                elif(respondendo):
                                    print("aqui2")
                                ...
                                if(pymedio >= bboxb[1] and pymedio <= bboxb[3]):
                                    ...
                                    "{{\\rtlch \\ltrch\\loch\\qc\\pict\\pngblip\\picscalex100\\picscaley100\\piccropl0\\piccropr0\\piccropt0\\piccropb0\\picw{}\\pich{}\n{}}}\\line"\
                                        .format(width, height, pngtohex)
                                                ...
                                                Image.LOAD_TRUNCATED_IMAGES = True #teste2
                                                with Image.open(filepath) as imx:
                                                   
                                                    imx.thumbnail(size, Image.LANCZOS)
                                                    imx.save('teste.png','PNG')
                                                    width, height = imx.size
                                                with open('teste.png', 'rb') as f: #teste
                                                    content = f.read()
                                                    pngtohex = binascii.hexlify(content).decode('utf8')
                                                    estaselecao += \
                                "{{\\rtlch \\ltrch\\loch\\qc\\pict\\pngblip\\picscalex100\\picscaley100\\piccropl0\\piccropr0\\piccropt0\\piccropb0\\picw{}\\pich{}\n{}}}\\line"\
                 ...
                 if(pmin!=None and pmax!=None and pmin!=pmax):
                     ...
                     docpagina = "{{\\fs22\\f2\\b{{ Relat}}\\\'D3rio \\\'22{}\\\'22 -- Fls. {}}}".format(docname, pagina+1)   
                 docpagina = docpagina.upper()
                 textonatabela += ("\\par\\trowd\\clbrdrb\\brdrs\\clbrdrt\\brdrs\\clbrdrl\\brdrs\\clbrdrr\\brdrs\\trautofit1\\intbl\\clftsWidth3\\clwWidth9070\\cellx9070{{\\cbpat2\\qc\\loch\\b\\fs22\\f2 TABELA }}{{\\hich\\af2\\af2\\loch\\f2\\f2\\b\\fs22\\loch\\field\\flddirty{{\\*\\fldinst{{SEQ Tabela\\\\* ARABIC }}}}{{\\fldrslt{{ }}}}}}"+\
                 "{{\\f2\\fs22\\b\\qc:{}}}\\f2\\fs20\\cell\\row"+\
                    # "\\trowd\\clftsWidth1\\clbrdrb\\brdrs\\clbrdrt\\brdrs\\clbrdrl\\brdrs\\clbrdrr\\brdrs\\cellx1\\intbl{{\\cbpat2\\qc\\loch\\b{{TABELA}}}}\\cell\\row"+\
                     "\\par\\trowd\\clbrdrb\\brdrs\\clbrdrt\\brdrs\\clbrdrl\\brdrs\\clbrdrr\\brdrs\\trautofit1\\intbl\\clftsWidth3\\clwWidth9070\\cellx9070 {}\\cell\\row\\pard\\line").format(docpagina, estaselecao) + "\n"
                 return (textonatabela, estaselecao)
         except Exception as ex:
    ...
    def sanitize_search(self, searchVar, entrysearch):
        
        
        if("\n" in searchVar.get()):
            #print(self.searchVar.get(), self.searchtypeVar.get())
            old = searchVar.get().replace("\n",' ').replace("\r",' ')
            entrysearch.delete("0", "end")
            entrysearch.insert("0", old)
    ...
    def create_search_frame(self): 
        ...
        self.searchVar.trace_add("write", lambda name, index, mode: self.sanitize_search(self.searchVar, self.entrysearch))
        ...
        self.limitsearchVar.set(global_settings.limit_search)
        limite = global_settings.limit_search
        if(limite <= 0):
            limite = "Sem Limite"
        self.limitsearchlabel = tkinter.Label(self.limitSearchFrame, font=global_settings.Font_tuple_Arial_10, text=f'*Max. Resultados por seção: {limite}')
        ...
        self.collapse = tkinter.Button(self.searchbuttonsframe, font=global_settings.Font_tuple_Arial_10, text='Colapsar todos', \
                                        image=global_settings.collapseimg, compound="right", command= lambda: self.collapseall(None, self.treeviewSearches))
        ...
        self.treeviewSearches.bind_class('post-tree-bind-search', "<1>", lambda e: self.treeview_selection_search(e, self.treeviewSearches, 
                                                                                                                  self.termosearchVar, self.ocorrenciasLabel))
        self.treeviewSearches.bind_class('post-tree-bind-search', "<3>", self.treeview_search_right)
        self.treeviewSearches.bind_class('post-tree-bind-search','<Up>', lambda e: self.treeview_selection_search(None, self.treeviewSearches, 
                                                                                                                  self.termosearchVar, self.ocorrenciasLabel))
        self.treeviewSearches.bind_class('post-tree-bind-search','<Down>', lambda e: self.treeview_selection_search(None, self.treeviewSearches, 
                                                                                                                    self.termosearchVar, self.ocorrenciasLabel))
        ...
        self.treeviewSearches.tag_configure('eqsearch', background='#f0f0f0',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearches.tag_configure('arqsearch', background='#f0f0f0',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearches.tag_configure('arqsearcherror', background='#db041a',font=global_settings.Font_tuple_Arial_8)
    ...
    def create_search_framefiles(self): 
        self.searchFrameFiles = tkinter.Frame(borderwidth=2, bg='white')
        
            
            
            
        self.searchFrameFiles.rowconfigure(2, weight=1)
        self.searchFrameFiles.columnconfigure(0, weight=1)
        self.searchBoxFiles = tkinter.Frame(self.searchFrameFiles, borderwidth=2, relief='ridge')
        self.searchBoxFiles.grid(row=0, column=0, sticky='new', pady=(0, 5))
        self.searchBoxFiles.rowconfigure((0,1,2), weight=1)
        self.searchBoxFiles.columnconfigure(0, weight=2)
        self.searchBoxFiles.columnconfigure(1, weight=1)
        self.searchVarFiles = tkinter.StringVar()
        self.searchVarFiles.set("")
        self.searchtypeVarFiles = tkinter.StringVar()
        self.searchtypeVarFiles.set("LIKE")
        
        
        self.entrysearchFiles = classes_general.PlaceholderEntry(self.searchBoxFiles, placeholder='Buscar...', justify='center', textvariable=self.searchVarFiles, state='normal',\
                                                            exportselection=False)
        self.searchVarFiles.trace_add("write", lambda name, index, mode: self.sanitize_search(self.searchVarFiles, self.entrysearchFiles))
        self.entrysearchFiles.bind('<Return>',  lambda e: self.searchTermFiles(event=e))
        self.entrysearchFiles.grid(row=1, column=0, sticky='nsew', padx=2, pady=5)            
        self.limitSearchFrameFiles = tkinter.Frame(self.searchBoxFiles)
        self.limitSearchFrameFiles.grid(row=0, column=0, sticky='w', pady=2)
        self.limitSearchFrameFiles.rowconfigure(0, weight=1)
        self.limitSearchFrameFiles.columnconfigure(0, weight=1)
        self.querysqlFiles = tkinter.Button(self.searchBoxFiles, font=global_settings.Font_tuple_Arial_10, text='', \
                                        image=global_settings.querysqlim, compound="right", command=self.querySql)
        self.querysqlFiles.grid(row=1, column=2, sticky='e', padx=5)
        self.searchbuttonFiles = tkinter.Button(self.searchBoxFiles, font=global_settings.Font_tuple_Arial_10, text='Pesquisar', \
                                            image=global_settings.lupa, compound="right", state='normal', command= lambda: self.searchTermFiles())
        self.searchbuttonFiles.grid(row=1, column=1, sticky='w', pady=2)
        self.searchtypeFrameFiles = tkinter.Frame(self.searchBoxFiles)
        self.searchtypeFrameFiles.grid(row=2, column=0, columnspan=2, pady=0, padx=0, sticky='nsew')
        self.searchtypeFrameFiles.rowconfigure(0, weight=1)
        self.searchtypeFrameFiles.columnconfigure((0,1,2), weight=1)
        
        self.searchtypeLikeFiles = tkinter.Radiobutton(self.searchtypeFrameFiles,variable=self.searchtypeVarFiles, font=global_settings.Font_tuple_Arial_10, text="LIKE", value="LIKE")
        self.searchtypeMatchFiles = tkinter.Radiobutton(self.searchtypeFrameFiles,variable=self.searchtypeVarFiles, font=global_settings.Font_tuple_Arial_10, text="MATCH", value="MATCH")
        self.searchtypeRegexFiles = tkinter.Radiobutton(self.searchtypeFrameFiles,variable=self.searchtypeVarFiles, font=global_settings.Font_tuple_Arial_10, text="REGEX", value="REGEX")
        self.searchtypeLikeFiles.grid(row=0, column=0, sticky='ns')
        self.searchtypeMatchFiles.grid(row=0, column=1, sticky='ns')
        self.searchtypeRegexFiles.grid(row=0, column=2,  sticky='ns')            
        
        self.paned_window_files_searches_and_browse = ttk.PanedWindow(self.searchFrameFiles, orient=tkinter.VERTICAL)
        self.paned_window_files_searches_and_browse.rowconfigure(0, weight=1)
        self.paned_window_files_searches_and_browse.columnconfigure(0, weight=1)
        self.paned_window_files_searches_and_browse.grid(row=2, column=0, sticky='nsew')
        self.searchtreeframeFiles = tkinter.Frame(borderwidth=2, relief='ridge')
        self.paned_window_files_searches_and_browse.add(self.searchtreeframeFiles, weight=1)
        self.searchtreeframeFiles.rowconfigure(0, weight=1)
        self.searchtreeframeFiles.columnconfigure(0, weight=1)
        self.treeviewSearchesFiles = ttk.Treeview(self.searchtreeframeFiles, selectmode='extended')
        self.treeviewSearchesFiles.bindtags(('.self.treeviewEqs', 'Treeview', 'post-tree-bind-search-files','.','all'))
        """ self.treeviewSearchesFiles.bind_class('post-tree-bind-search-files', "<1>", 
                                              lambda e: self.treeview_selection_search(e, self.treeviewSearchesFiles, 
                                                                                       None, None))
        #self.treeviewSearchesFiles.bind_class('post-tree-bind-search-files', "<3>", self.treeview_search_right)
        self.treeviewSearchesFiles.bind_class('post-tree-bind-search-files','<Up>', 
                                              lambda e: self.treeview_selection_search(None, self.treeviewSearchesFiles, 
                                                                                       None, None))
        self.treeviewSearchesFiles.bind_class('post-tree-bind-search-files','<Down>', 
                                              lambda e: self.treeview_selection_search(None, self.treeviewSearchesFiles, 
                                                                                       None, None)) """
        self.treeviewSearchesFiles.heading("#0", text="Buscas", anchor="w")
        self.treeviewSearchesFiles.column("#0", width=200, stretch=True, minwidth=200, anchor="w")
        self.treeviewSearchesFiles.grid(row=0, column=0, sticky='nsew')
        self.scrolltreeviewSearchesFiles = ttk.Scrollbar(self.searchtreeframeFiles, orient="vertical")
        self.scrolltreeviewSearchesFiles.grid(row=0, column=1, sticky='ns')
        self.scrolltreeviewSearchesFiles.config( command = self.treeviewSearchesFiles.yview )
        self.treeviewSearchesFiles.tag_configure('termosearch', foreground="#000000", background='#d0d0d0', font=global_settings.Font_tuple_ArialBoldUnderline_10)
        self.treeviewSearchesFiles.tag_configure('termosearching', foreground='#d0d0d0', font=global_settings.Font_tuple_ArialBold_10)
        self.treeviewSearchesFiles.tag_configure('termoinprocess', background='#7dffd8', font=global_settings.Font_tuple_ArialBold_10)
        self.treeviewSearchesFiles.tag_configure('termowitherror', background='#db041a', font=global_settings.Font_tuple_ArialBold_10)
        self.treeviewSearchesFiles.tag_configure('resultsearch',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFiles.tag_configure('relsearch', background='#f0f0f0',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFiles.tag_configure('eqsearch', background='#f0f0f0',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFiles.tag_configure('arqsearch', background='#f0f0f0',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFiles.tag_configure('arqsearcherror', background='#db041a',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFiles.tag_configure('relsearchtoc',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFiles.configure(yscrollcommand=self.scrolltreeviewSearchesFiles.set)
        self.scrolltreeviewSearchesHFiles = ttk.Scrollbar(self.searchtreeframeFiles, orient="horizontal")
        self.scrolltreeviewSearchesHFiles.config(command = self.treeviewSearchesFiles.xview )
        self.treeviewSearchesFiles.configure(xscrollcommand=self.scrolltreeviewSearchesHFiles.set)
        self.scrolltreeviewSearchesHFiles.grid(row=1, column=0, sticky='ew')
        
        self.searchtreeframeFilesBrowse = tkinter.Frame(borderwidth=2, relief='ridge')
        self.paned_window_files_searches_and_browse.add(self.searchtreeframeFilesBrowse, weight=1)
        self.searchtreeframeFilesBrowse.rowconfigure(0, weight=1)
        self.searchtreeframeFilesBrowse.columnconfigure(0, weight=1)
        self.treeviewSearchesFilesBrowse = ttk.Treeview(self.searchtreeframeFilesBrowse, selectmode='extended')
        self.treeviewSearchesFilesBrowse.bindtags(('.self.treeviewEqs', 'Treeview', 'post-tree-bind-search-files','.','all'))
        """ self.treeviewSearchesFilesBrowse.bind_class('post-tree-bind-search-files', "<1>", 
                                              lambda e: self.treeview_selection_search(e, self.treeviewSearchesFilesBrowse, 
                                                                                       None, None))
        #self.treeviewSearchesFiles.bind_class('post-tree-bind-search-files', "<3>", self.treeview_search_right)
        self.treeviewSearchesFilesBrowse.bind_class('post-tree-bind-search-files','<Up>', 
                                              lambda e: self.treeview_selection_search(None, self.treeviewSearchesFilesBrowse, 
                                                                                       None, None))
        self.treeviewSearchesFilesBrowse.bind_class('post-tree-bind-search-files','<Down>', 
                                              lambda e: self.treeview_selection_search(None, self.treeviewSearchesFilesBrowse, 
                                                                                       None, None)) """
        self.treeviewSearchesFilesBrowse.heading("#0", text="Diretórios", anchor="w")
        self.treeviewSearchesFilesBrowse.column("#0", width=200, stretch=True, minwidth=200, anchor="w")
        self.treeviewSearchesFilesBrowse.grid(row=0, column=0, sticky='nsew')
        self.scrolltreeviewSearchesFilesBrowse = ttk.Scrollbar(self.searchtreeframeFilesBrowse, orient="vertical")
        self.scrolltreeviewSearchesFilesBrowse.grid(row=0, column=1, sticky='ns')
        self.scrolltreeviewSearchesFilesBrowse.config( command = self.treeviewSearchesFilesBrowse.yview )
        self.treeviewSearchesFilesBrowse.tag_configure('termosearch', foreground="#000000", background='#d0d0d0', font=global_settings.Font_tuple_ArialBoldUnderline_10)
        self.treeviewSearchesFilesBrowse.tag_configure('termosearching', foreground='#d0d0d0', font=global_settings.Font_tuple_ArialBold_10)
        self.treeviewSearchesFilesBrowse.tag_configure('termoinprocess', background='#7dffd8', font=global_settings.Font_tuple_ArialBold_10)
        self.treeviewSearchesFilesBrowse.tag_configure('termowitherror', background='#db041a', font=global_settings.Font_tuple_ArialBold_10)
        self.treeviewSearchesFilesBrowse.tag_configure('resultsearch',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFilesBrowse.tag_configure('relsearch', background='#f0f0f0',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFilesBrowse.tag_configure('eqsearch', background='#f0f0f0',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFilesBrowse.tag_configure('arqsearch', background='#f0f0f0',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFilesBrowse.tag_configure('arqsearcherror', background='#db041a',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFilesBrowse.tag_configure('relsearchtoc',font=global_settings.Font_tuple_Arial_8)
        self.treeviewSearchesFilesBrowse.configure(yscrollcommand=self.scrolltreeviewSearchesFilesBrowse.set)
        self.scrolltreeviewSearchesHFilesBrowse = ttk.Scrollbar(self.searchtreeframeFilesBrowse, orient="horizontal")
        self.scrolltreeviewSearchesHFilesBrowse.config(command = self.treeviewSearchesFilesBrowse.xview )
        self.treeviewSearchesFilesBrowse.configure(xscrollcommand=self.scrolltreeviewSearchesHFilesBrowse.set)
        self.scrolltreeviewSearchesHFilesBrowse.grid(row=1, column=0, sticky='ew')
        
        #self.new_files_frame = tkinter.Frame(self.swapframes, bg=self.bg, highlightthickness=0)
        #self.new_files_frame.rowconfigure(0, weight=1)
        #self.new_files_frame.columnconfigure(0, weight=1)
        self.files_paned_window = files_frame.CustomPanedWindow(self.swapframes, global_settings.columns_files)
        self.files_paned_window.grid(row=0, column=0, sticky='nsew', padx=0, pady=0)
    ...
    def _on_tab_change(self, event):
        ...
        #print(selected_tab)
        if selected_tab == "Arquivos":
            self.files_paned_window.lift()  # Bring frame 1 to the front
            self.current_top_frame = self.files_paned_window
            self.is_onfiles_frame = True
        else:
            self.files_paned_window.lower()  # Bring frame 1 to the front
            self.current_top_frame = self.swapframes
            self.is_onfiles_frame = False
        """ elif selected_tab == "Hexadecimal":
    ...
    def leftPanel(self):
        ...
        try:
            ...
            #self.searchFrameMain = tkinter.Frame(borderwidth=2, bg='white')
            #self.searchFrameMain.rowconfigure(0, weight=1)
            #self.searchFrameMain.columnconfigure(0, weight=1)
            #self.notebookSearches = ttk.Notebook(self.searchFrameMain, padding=8) 
            #self.notebookSearches.grid(row=0, column=0, sticky='nsew')
            ...
            self.notebook.add(self.tocFrame, text="Relatórios", sticky='nsew', image=global_settings.repicon, compound='top')
            self.notebook.add(self.searchFrame, text="Buscas", sticky='nsew', compound='top', image=global_settings.searchicon)
            ...
            try:
                cursor = sqliteconn.cursor()   
                check_previous_search =  "SELECT DISTINCT C.termo, C.tipobusca, C.id_termo, C.fixo, C.pesquisado, C.tipo  FROM Anexo_Eletronico_SearchTerms C ORDER by 3"
                ...
                for termox in termos:
                    ...
                    if(tipofonte=='relatorio'):
                        ...
                        global_settings.contador_buscas_incr += 1
                    if(tipofonte=='arquivo'):
                        global_settings.listaTERMOSFiles[(termo.upper(),tipobusca)] = [termo,tipobusca, idtermo, pesquisados]
                        pedidosearch = classes_general.PedidoSearch(int(idtermo)*-1, tipobusca, termo)
                        global_settings.searchqueueFiles.put(pedidosearch)
                        #global_settings.searchqueue.append((termo, tipobusca, None))
                        global_settings.contador_buscas_incrFiles += 1
    ...
    def collapseall(self, event=None, treeview=None):
    ...
    def iterateSearchList(self, event=None, tipo=None):
        try:
            ...
            self.treeview_selection_search(None, self.treeviewSearches)            
        except Exception as ex:
            utilities_general.printlogexception(ex=ex)            
            
    def treeview_selection_search(self, event=None, treeview=None, termosearchVar=None, ocorrenciasLabel=None):        
        try:
            region = ""
            if(event!=None):
            ...
            if region == "heading":
                ...
                for pdf in global_settings.infoLaudo:
                    global_settings.infoLaudo[pdf].retangulosDesenhados = {}
                if(event!=None):
                    searchresultiid = treeview.identify_row(event.y)
                else:
                    searchresultiid = treeview.selection()[0]
                try:
                    resultsearch = global_settings.searchResultsDict[searchresultiid]
                except Exception as ex:
                    return                
                parent = treeview.parent(searchresultiid)
                if(len(treeview.get_children(searchresultiid))==0 and parent != ''):                      
                    raiz = treeview.parent(searchresultiid)                    
                    while(raiz!=''):
                        if(treeview.parent(raiz)==''):
                            break
                        raiz = treeview.parent(raiz)
                    ...
                    totalhits = treeview.item(parent, 'text').split(' ')
                    ocorrenciasLabel.config(font=global_settings.Font_tuple_Arial_10, text=str(treeview.index(searchresultiid)+1) + ' de ' + totalhits[len(totalhits)-1])
                    if(termosearchVar!=None):
                        termosearchVar.set(treeview.item(raiz, 'text'))
                    self.docInnerCanvas.delete("simplesearch")
        ...
        if(len(listaresultados)>0):            
            ...
            if(pagina not in global_settings.processed_pages):
                ...
                for resultsearch in reversed(listaresultados):
                    ...
                    p0x, p0y, p1x, p1y = None, None, None, None
                    if(resultsearch.link_position!=None):
                        p0x, p0y, p1x, p1y = resultsearch.link_position
                    else:
    ...
    def searchTerm(self, termo=None, event=None, tipobusca=None, testar=False):
        try:
            if(termo==None):
            ...
            if(testar):
                global_settings.allok = 771
                print(traceback.format_exc())
            utilities_general.printlogexception(ex=ex)
            
    def searchTermFiles(self, termo=None, event=None):
        try:
            if(termo==None):
                termo = self.searchVarFiles.get().strip()
            novotermo = ""
            for char in termo:
                codePoint = ord(char)
                if(codePoint<256):
                ...
                novotermo += chr(codePoint) 
            novotermo = novotermo.strip()
            tipobusca = self.searchtypeVarFiles.get()
            if tipobusca=="REGEX":
                termo = termo
            elif tipobusca=="MATCH":
                termo = termo
                ...
                termo = novotermo
            if(not (termo.upper(), tipobusca) in global_settings.listaTERMOSFiles):
                global_settings.listaTERMOSFiles[(termo.upper(),tipobusca)] = []
                pedidosearch = classes_general.PedidoSearch(-math.inf, tipobusca, termo)
                global_settings.searchqueueFiles.put(pedidosearch)
                #global_settings.searchqueue.insert(0, (termo, tipobusca, None, '0')) 
                global_settings.contador_buscas_incrFiles += 1                
            elif(len(global_settings.listaTERMOSFiles[termo.upper(), tipobusca])>0):
                idtermo = str(global_settings.listaTERMOSFiles[(termo.upper(),tipobusca)][2])
                self.treeviewSearchesFiles.selection_set('t'+idtermo)
                self.treeviewSearchesFiles.move('t'+idtermo, '', 0)
                self.treeviewSearchesFiles.focus('t'+idtermo)                    
        except Exception as ex:
            utilities_general.printlogexception(ex=ex)
    ...
    def createTopBar(self):      
        try:
            ...
            self.globalFrame.add(self.swapframes, weight=1)  
            
            self.create_search_framefiles()     
            #self.notebook.add(self.searchFrameFiles, text="Arquivos", sticky='nsew', compound='top', image=global_settings.searchfilesicon)          
    ...
    def menuReports(self, event=None):
        try:
            ...
            self.reportbut.menu.add_separator()
            self.reportbut.menu.add_command(label='Sobre', image=global_settings.aboutimage , compound='left', command= self.show_about)
            self.reportbut.pack()    
...
def start_fera_app(testar1=False, testar2=False):
    start_time = time.time()
    try:
        ...
        try:
            ...
            global_settings.splash_window.label.update()
            global_settings.fera_main_window = MainWindow(testar1=testar1, testar2=testar2) 
            
            
                
        except Exception as ex:
            global_settings.allok = 773
            utilities_general.printlogexception(ex=ex)
        try:
            global_settings.splash_window.window.destroy()
        except:
